...
// (index.html에서 <script type="module">). storage.js는 카카오맵 SDK
// 로딩 타이밍 때문에 계속 클래식 스크립트로 두고, 여기서는 클라이언트
// 생성 함수만 window에 얹어 storage.js가 필요할 때(Storage.init() 안에서)
// 가져다 쓰게 한다.
...
// storage.js가 이 Promise를 기다리게 한다.)
fetch(OUTPUTS_FILE)
  .then((r) => r.json())
  .then((outputs) => {
  ...
  .catch((e) => window._backendClientReadyResolvers.reject(e));
